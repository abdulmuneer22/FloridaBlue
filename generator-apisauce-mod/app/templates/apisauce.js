...
var create = function create(config) {
  ...
  var convertResponse = function convertResponse(startedAt, axiosResponse) {
    ...
    var responseURL = response && response.request.responseURL || null;

    // give an opportunity for anything to the response transforms to change stuff along the way
    if (responseTransforms.length > 0) {
      R.forEach(function (transform) {
        transform({ duration: duration, problem: problem, ok: ok, status: status, headers: headers, config: config, data: data, responseURL: responseURL });
      }, responseTransforms);
    }

    return { duration: duration, problem: problem, ok: ok, status: status, headers: headers, config: config, data: data, responseURL: responseURL };
  };
